...
def get_list_authors(book_id: int):
    author_ids = list(
        map(lambda x: x.author_id, BookDetail.query.filter_by(book_id=book_id).all())
    )

    authors_names = Author.query.filter(Author.id.in_(author_ids)).all()
    authors_names = list(
        map(lambda x: x.get_data(["full_name"])["full_name"], authors_names)
    )
    return authors_names


def get_list_genres(book_id: int):
    book_genre_id = (
        BookGenre.query.filter_by(book_id=book_id)
        .with_entities(BookGenre.genre_id)
        .all()
    )
...
def get_book(user_id: int, book_id: int):
    """
    return information of a book of a user
    """
    try:
        book = Book.query.filter_by(id=book_id).first().get_data()

        book_review = BookReview.query.filter_by(book_id=book_id).first().get_data()

        ret_data = book
        ret_data.update(book_review)
        ret_data["authors"] = get_list_authors(book_id)
        ret_data["genres"] = get_list_genres(book_id)

        if user_id:  # user id not None
            try:
                ret_data["user_rating"] = (
                    BookRating.query.filter_by(user_id=user_id, book_id=book_id)
                    .first()
                    .rating
                )
            ...
            try:
                ret_data["is_favorite"] = (
                    BookFavorite.query.filter_by(user_id=user_id, book_id=book_id)
                    .first()
                    .is_favorite
                )
            except AttributeError:
        ...
        return make_response(
            make_data(dict(error=str(e)), msg="Return a book fail!", status="FAILURE")
        )
...
def get_all_authors():
    try:
        ...
        return make_response(
            make_data(
                dict(error=str(e)), msg="Return list authors fail!", status="FAILURE"
            )
        )

    return make_response(
        make_data(
            dict(list_authors=list_authors), msg="Return list authors successfully!"
        )
    )
...
def get_all_user():
    ...
    try:
        ...
        return make_response(
            make_data(dict(error=str(e)), msg="Return list user fail", status="FAILURE")
        )

    return make_response(
        make_data(dict(list_users=list_users), msg="Return list user successfully!")
    )
...
def get_all_genres():
    try:
        ...
        return make_response(
            make_data(
                dict(error=str(e)), msg="Return list genres fail!", status="FAILURE"
            )
        )

    return make_response(
        make_data(dict(list_genres=list_genres), msg="Return list genres successfully!")
    )


@app.route(
    "/api/update-count-author-genre/<int:user_id>/<int:book_id>", methods=["PUT"]
)
@cross_origin()
def update_count_author_genre(user_id: int, book_id: int):
    ...
    try:
        author_ids = list(
            map(
                lambda x: x[0],
                BookDetail.query.filter_by(book_id=book_id)
                .with_entities(BookDetail.author_id)
                .all(),
            )
        )
        ...
        try:
            ...
            author_count = AuthorCount.query.filter(
                AuthorCount.user_id == user_id, AuthorCount.author_id.in_(author_ids)
            ).all()
            assert author_count
            ...
            list_author_count = list(
                map(
                    lambda auth_id: AuthorCount(user_id=user_id, author_id=auth_id),
                    author_ids,
                )
            )
        ...
        try:
            ...
            genre_count = GenreCount.query.filter(
                GenreCount.user_id == user_id, GenreCount.genre_id.in_(genre_ids)
            ).all()
            assert genre_count
            ...
            # when not row queried
            list_genre_count = list(
                map(
                    lambda genre_id: GenreCount(user_id=user_id, genre_id=genre_id),
                    genre_ids,
                )
            )
        ...
        return make_response(
            make_data(
                dict(error=str(e)),
                msg="Update author and genre count fail!",
                status="FAILURE",
            )
        )
...
def get_book_similar(book_id: int):
    try:
        ...
        weighted_rating = (
            BookReview.query.filter(BookReview.book_id.in_(similar_books))
            .with_entities(BookReview.weighted_rating)
            .all()
        )

        similar_books = list(
            map(
                lambda x: Book.query.filter_by(id=x)
                .first()
                .get_data(cols=["id", "image_url", "title"]),
                similar_books,
            )
        )
        similar_books = [
            dict(item, rating=rating[0])
            for item, rating in zip(similar_books, weighted_rating)
        ]
        ...
        return make_response(
            make_data(
                dict(error=str(e)),
                msg="Return top 10 book similar fail!",
                status="FAILURE",
            )
        )
    ...
    return make_response(
        make_data(
            dict(list_book=similar_books),
            msg="Return top 10 book similar successfully!",
        )
    )
...
def get_top_100_books():
    ...
    try:
        list_book_data = (
            BookReview.query.order_by(BookReview.weighted_rating.desc())
            .with_entities(BookReview.book_id, BookReview.weighted_rating)
            .limit(LIMIT_BOOKS)
            .all()
        )
        ...
        list_book = list(
            map(
                lambda x: x.get_data(cols=["id", "image_url", "title"]),
                Book.query.filter(Book.id.in_(list_book_id)).all(),
            )
        )

        list_book = [
            dict(item, rating=rating[1])
            for item, rating in zip(list_book, list_book_data)
        ]

    except Exception as e:
        return make_response(
            make_data(
                dict(error=str(e)), msg="Return top 100 books fail!", status="FAILURE"
            )
        )

    return make_response(
        make_data(
            data=dict(list_book=list_book), msg="Return top 100 books successfully!"
        )
    )


@app.route(
    "/api/update-user-rating/<int:user_id>/<int:book_id>/<int:user_rating>",
    methods=["PUT"],
)
@cross_origin()
def update_user_rating(user_id: int, book_id: int, user_rating: int):
    try:
        ...
        book_rating = BookRating(rating=user_rating, user_id=user_id, book_id=book_id)
...
def get_list_book_rated(user_id: int):
    try:
        book_rating = (
            BookRating.query.filter_by(user_id=user_id)
            .filter(BookRating.rating > 0)
            .with_entities(BookRating.book_id, BookRating.rating)
            .all()
        )
        list_book = list(
            map(
                lambda x: Book.query.filter(Book.id == x[0])
                .first()
                .get_data(["id", "image_url", "title"]),
                book_rating,
            )
        )
        list_book = [
            dict(item, rating=rating[1]) for item, rating in zip(list_book, book_rating)
        ]
    except Exception as e:
        return make_response(
            make_data(
                data=dict(error=str(e)),
                msg="Return list book rated fail!",
                status="FAILURE",
            )
        )
    return make_response(
        make_data(
            data=dict(list_book=list_book), msg="Return list book rated successfully!"
        )
    )
...
def get_book_favorited(user_id: int):
    try:
        favorited_book_ids = list(
            map(
                lambda x: x[0],
                BookFavorite.query.filter_by(user_id=user_id, is_favorite=True)
                .with_entities(BookFavorite.book_id)
                .all(),
            )
        )

        weighted_rating = (
            BookReview.query.filter(BookReview.book_id.in_(favorited_book_ids))
            .with_entities(BookReview.weighted_rating)
            .all()
        )

        favorited_books = list(
            map(
                lambda x: x.get_data(cols=["id", "image_url", "title"]),
                Book.query.filter(Book.id.in_(favorited_book_ids)).all(),
            )
        )
        favorited_books = [
            dict(item, rating=rating[0])
            for item, rating in zip(favorited_books, weighted_rating)
        ][::-1]

    except Exception as e:
        return make_response(
            make_data(
                dict(error=str(e)), msg="Return favorited book fail!", status="FAILURE"
            )
        )

    return make_response(
        make_data(
            data=dict(list_book=favorited_books),
            msg="Return favorited book sucessfully!",
        )
    )
...
def create_user():
    ...
    try:
        ...
        book_review = BookReview.query.options(
            load_only(*["book_id", "weighted_rating"])
        ).all()
        ...
        return make_response(
            make_data(dict(error=str(e)), msg="Create user fail!", status="FAILURE")
        )

    return make_response(
        make_data(dict(user_id=user.id), msg="Create user successfully!")
    )


@app.route("/api/get-user/<path:uid>")
@cross_origin()
def get_user(uid: str):

    try:
        user = User.query.filter(User.uid == uid).first().get_data()
    except:
        return make_response(
            make_data(
                dict(error=str(e)), msg="Return user info fail!", status="FAILURE"
            )
        )
...
def get_reading_list_history(user_id):
    ...
    try:
        ...
        list_book = list(
            map(
                lambda x: x.get_data(cols=["id", "image_url", "title"]),
                Book.query.filter(Book.id.in_(book_rating_ids)).all(),
            )
        )
        list_book = [
            dict(item, rating=rating[1]) for item, rating in zip(list_book, book_rating)
        ...
        return make_response(
            make_data(
                dict(error=str(e)),
                msg="Return reading list history fail",
                status="FAILURE",
            )
        )
    ...
    return make_response(
        make_data(
            dict(list_book=list_book), msg="Return reading list history successfully!"
        )
    )


@app.route(
    "/api/get-list-books-by-author-genre/<int:author_id>/<int:genre_id>",
    methods=["GET"],
)
@cross_origin()
def get_list_books_by_author_genre(author_id: int, genre_id: int):
    ...
    try:
        ...
        list_book_id_1 = (
            BookDetail.query.filter_by(author_id=author_id)
        ...
        list_books = (
            Book.query.filter(Book.id.in_(list_match_book_ids)).limit(LIMIT_BOOK).all()
        )
        list_books = list(
            map(lambda x: x.get_data(cols=["id", "image_url", "title"]), list_books)
        )
        ...
        list_books = [
            dict(
                item,
                rating=rating[0],
                authors=get_list_authors(item["id"]),
                genres=get_list_genres(item["id"]),
            )
            for item, rating in zip(list_books, weighted_rating)
        ]
    ...
    return make_response(
        make_data(
            dict(no_books=len(list_books), list_book=list_books),
            msg="Return list genres successfully!",
        )
    )
...
def get_list_book_recommend_by_author(user_id: int):
    ...
    try:
        ...
        user_rating_list = BookRating.query.filter(
            and_(BookRating.user_id == user_id, BookRating.rating > 0)
        ).all()
        ...
        recommend_info = list(
            map(
                lambda x: x.get_data(["book_id", "author_weight"]),
                recommend_info_origin,
            )
        )
        ...
        list_book_final = [
            book_id
            for weight, book_id in sorted(
                zip(y_pred, list_book_id), reverse=True, key=lambda x: x[0]
            )
        ][:LIMIT_BOOK]
        weighted_rating = (
            BookReview.query.filter(BookReview.book_id.in_(list_book_final))
            .with_entities(BookReview.weighted_rating)
            .all()
        )
        ...
        list_book = list(
            map(lambda x: x.get_data(["id", "image_url", "title"]), list_book)
        )

        list_book = [
            dict(item, rating=rating[0])
            for item, rating in zip(list_book, weighted_rating)
        ]
        ...
        return make_response(
            make_data(
                dict(error=str(e)),
                msg="Return list recommend books by author fail!",
                status="FAILURE",
            )
        )
    ...
    return make_response(
        make_data(
            dict(list_book=list_book),
            msg="Return list recommend books by author successfully!",
        )
    )
...
def get_list_book_recommend_by_genre(user_id: int):
    ...
    try:
        ...
        recommend_info = list(
            map(
                lambda x: x.get_data(["book_id", "genre_weight"]), recommend_info_origin
            )
        )
        ...
        return make_response(
            make_data(
                dict(error=str(e)),
                msg="Return list recommend books by genre fail!",
                status="FAILURE",
            )
        )
    ...
    return make_response(
        make_data(
            dict(list_book=list_book),
            msg="Return list recommend books by genre successfully!",
        )
    )
...
def get_some_book_similar_at_all(user_id: int):
    ...
    try:
        book_similar_id = (
            BookRating.query.filter_by(user_id=user_id)
            .with_entities(BookRating.book_id)
            .all()
        )
        book_similar_id = list(map(lambda x: x[0], book_similar_id))

        book_similar_id = (
            BookDescriptionSimilarities.query.filter(
                BookDescriptionSimilarities.book_id.in_(book_similar_id)
            )
            .with_entities(BookDescriptionSimilarities.book_recommend_id)
            .all()
        )
        ...
        weighted_rating = (
            BookReview.query.filter(BookReview.book_id.in_(book_similar_id))
            .with_entities(BookReview.weighted_rating)
            .all()
        )
        ...
        return make_response(
            make_data(
                dict(error=str(e)),
                msg="Return top similar book fail!",
                status="FAILURE",
            )
        )
    ...
    return make_response(
        make_data(
            dict(list_book=list_book), msg="Return top similar book successfully!"
        )
    )
...
def get_one_top_book():
    try:
        ...
        return make_response(
            make_data(
                dict(error=str(e)), msg="Return top one book fail!", status="FAILURE"
            )
        )

    return get_book(None, top_book_id)
