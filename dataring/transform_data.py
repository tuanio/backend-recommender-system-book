...
table_temp = master_file[['isbn', 'isbn13', 'title', 'desc', 'pages', 'img', 'link']].copy()

table_temp.columns = ['isbn', 'isbn13', 'title', 'desc', 'pages', 'image_url', 'book_url']

list_books = [Book(**BookDto(**dict(table_temp.iloc[i, :])).dict()) for i in range(table_temp.shape[0])]
...
book_review_list = [BookReview(**dict(book_review.iloc[i, :])) for i in range(book_review.shape[0])]
...
for idx in range(BOOK.shape[0]):
    ...
    for au in list_authors:
        ...
        BOOK_DETAIL['book_id'].append(idx + 1)    # plus 1 to same to id in database
        BOOK_DETAIL['author_id'].append(au_id)

book_id, author_id = BOOK_DETAIL['book_id'], BOOK_DETAIL['author_id']
list_book_detail = [BookDetail(book_id=bo_id, author_id=au_id) for bo_id, au_id in zip(book_id, author_id)]
...
list_genre = [BookGenre(book_id=bo_id, genre_id=gen_id) for bo_id, gen_id in zip(book_id, genre_id)]
...
def getFormatId(format):
    ...
    default_id = 'nan'
    global db
    id = [i for i in db.session.query(BookFormat.id).filter_by(type_=format).all()]
    if len(id) != 0:
        return id[0][0]
    return  default_id
...
list_format_detail = [BookFormatDetail(book_id=bo_id, book_format_id=bo_f_id) for bo_id, bo_f_id in zip(book_id, book_format_id)]
